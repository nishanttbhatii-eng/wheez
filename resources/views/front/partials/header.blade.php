  ...
  <div class="header__top">
    <div class="container header__inner">
      ...
      <div class="header__center" id="navMenu">
          <nav class="nav" aria-label="{{ __('ui.main_nav') }}">
            <ul class="nav__list">
      ...
      <div class="header__right">
        @php
          $currentLocale = $displayLocale ?? session('locale', 'en');
          $langLabel = $currentLocale === 'hi' ? 'IN HI' : 'GB EN';
        @endphp
        <div class="lang-selector js-lang-selector notranslate" aria-label="Language" translate="no">
          <button
            type="button"
            ...
            id="langSelectorBtn"
          >
            <span class="lang-selector__text">{{ $langLabel }}</span>
            <svg class="lang-selector__arrow" width="10" height="10" viewBox="0 0 10 10" aria-hidden="true">
              <path d="M1.5 3.5L5 6.5L8.5 3.5" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </button>
          <ul class="lang-selector__menu" id="langMenu" role="listbox" hidden>
            <li role="option" aria-selected="{{ $currentLocale === 'en' ? 'true' : 'false' }}">
              <a
                href="{{ route('locale.switch', 'en') }}"
                class="lang-selector__option {{ $currentLocale === 'en' ? 'is-active' : '' }}"
                hreflang="en"
                data-locale="en"
              >
                <span class="lang-selector__flag" aria-hidden="true">🇬🇧</span>
                English
              </a>
            </li>
            <li role="option" aria-selected="{{ $currentLocale === 'hi' ? 'true' : 'false' }}">
              <a
                href="{{ route('locale.switch', 'hi') }}"
                class="lang-selector__option {{ $currentLocale === 'hi' ? 'is-active' : '' }}"
                hreflang="hi"
                data-locale="hi"
              >
                <span class="lang-selector__flag" aria-hidden="true">🇮🇳</span>
        ...
        <a href="https://wa.me/919625432342?text={{ urlencode(__('ui.wa_prefill')) }}" class="btn-whatsapp" aria-label="WhatsApp" target="_blank" rel="noopener">
